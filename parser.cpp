...
static std::unique_ptr<ExprAST> ParseSExpr() {
  ...
  if (CurTok == IF) {
    ...
    if (cnd && thn && els)
      return std::make_unique<IfExprAST>(OpPos, std::move(cnd), std::move(thn), std::move(els));
  }
  ...
  if (CurTok == SEQ) {
    ...
    if (fst && snd)
      return std::make_unique<SeqExprAST>(OpPos, std::move(fst), std::move(snd)); 
  }
  ...
  if (CurTok == SET) {
    ...
    if (val && var)
      return std::make_unique<SetExprAST>(OpPos, std::move(val), std::move(var));
  }
  ...
  if (CurTok == WHILE) {
    ...
    if (cnd && body)
      return std::make_unique<WhileExprAST>(OpPos, std::move(cnd), std::move(body));
  }
  ...
  if (CurTok == OP) {
    ...
    if (lhs && rhs)
      return std::make_unique<BinaryOpExprAST>(OpPos, thisOp, std::move(lhs), std::move(rhs));
  }
